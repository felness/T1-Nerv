...
def extract_and_classify_text_regions(image_path, reader, clip_model, clip_processor):
    ...
    for bbox, text, prob in results:
        ...
        regions.append({
            ...
            'label': predicted_label.replace(" text", ""),
...
def apply_nms(regions, iou_threshold=0.3):
    ...
    boxes_tensor = torch.tensor(boxes, dtype=torch.float32).to('cuda')
    scores_tensor = torch.tensor(scores).to('cuda')

    keep_indices = torchvision.ops.nms(boxes_tensor, scores_tensor, iou_threshold)
    filtered = [regions[i] for i in keep_indices]
    return filtered

def get_relevance_extracted_text(image_path, reader, clip_model, clip_processor, text_pipeline):
    regions = extract_and_classify_text_regions(image_path, reader, clip_model, clip_processor)

    images = [region['image'] for region in regions]

    results = text_pipeline(images)
